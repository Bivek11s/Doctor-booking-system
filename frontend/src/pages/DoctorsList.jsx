...
const DoctorsList = () => {
  ...
  useEffect(() => {
    fetchDoctors(specialty, searchQuery);
  }, [specialty, searchQuery, verificationFilter]);
  ...
  const fetchDoctors = async (
    ...
    try {
      ...
      // Build unique specialties list only once
      if (!specialtyParam && response.data.users.length) {
        const uniqueSpecialties = [
          ...new Set(
            response.data.users
              .map((doc) => doc.doctorSpecialty)
              .filter((s) => s)
          ),
        ];
        setSpecialties(uniqueSpecialties);
  ...
  const handleSpecialtyChange = (value) => {
    setSpecialty(value);
    if (value === "") {
      navigate("/doctors"); // Clear URL query
    }
  };
